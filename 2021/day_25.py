...
You don't have enough stars to boost the signal, though. You need 35 more.
...
def move_horizontal( data ):
	slugs_moved = False

	for row in range( 0, len( data ) ):
		col = 0
		initial_col = data[ row ][ 0 ]
		while col < len( data[ row ] ):
			...
			data[ row ].rotate( -1 )
	# draw( data )
	return slugs_moved, data


def move_vertical( data ):
	output = ''
	slugs_moved = False

	initial_row = list( data[ 0 ] )
	for col in range( 0, len( data[ 0 ] ) ):
		output = '[Col {0}] '.format( col )
		row = 0
...
def main( data ):
	...
	seabed = deque( [ deque( list( x ) ) for x in data ] )
	if DEBUG:
		print( 'Step {0:3} 0123456789'.format( steps ) )
		draw( seabed )

	while result:
		result_horizontal, seabed = move_horizontal( seabed )
		result_vertical, seabed = move_vertical( seabed )
	...
	print( 'The sea cucumbers stop moving after {0} steps.'.format( steps ) )


if __name__ == "__main__":
	input = r'D:\Projects\Advent_of_Code\2021\day_25_input.txt'
