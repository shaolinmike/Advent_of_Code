...
def pick_move(elf_move, player_strategy):
    ...
    #Initialize deque to elf's move
    move_list.rotate(elf_move_idx)
...
def resolve_turn(elf_move, player_move):
    ...
    if elf_move_idx == player_move_idx:
        score += DRAW
        if _DEBUG: print(f'\tYou tied. {elf_move} : {player_move}')

    elif elf_move_idx == ROCK and player_move_idx == SCISSORS:
        score += LOSE
        if _DEBUG: print(f'\tYou lose. {elf_move} beats {player_move}')

    elif elf_move_idx == SCISSORS and player_move_idx == ROCK:
        score += WIN
        if _DEBUG: print(f'\tYou win. {elf_move} loses to {player_move}')
