...
def find_prime_bus_numbers( busses ):
	found = [ ]

	for bus in busses:
		# Iterate from 2 to n / 2
		num = bus.id + bus.offset
...
def verify_arrival_with_bus_driver( busses, time ):
	...
	# output = '{0}\t'.format( time )
	# for bus in busses:
		# if time % bus.id != 0:
			# output += '\t[ {0} ] : {1}\t\t'.format( bus.id, time / bus.id % 60 + bus.offset )
			# # return False
		# else:
			# output += '\t[ {0} ] : {1}*\t\t'.format( bus.id, time / bus.id % 60 + bus.offset )

	# print( output )
	# print( '{0} *-- [ {1} ] : {2}\t\t[ {3} ] : {4}'.format( time, busses[ 0 ].id, bus_time, bus.id, bus.id * time % 60 + bus.offset ) )

	output = '{0}\t'.format( time )
	for bus in busses:
		if ( time + bus.offset ) % bus.id != 0:
			output += '\t[ {0} ] : {1}\t'.format( bus.id, round( time / bus.id % 60 + bus.offset, 2 ) )
			return False
		else:
			output += '\t[ {0} ] : {1}\t'.format( bus.id, round( time / bus.id % 60 + bus.offset, 2 ) )
			is_valid = True
...
def build_better_bus_schedule( raw_data ):
	...
	busses = create_busses( raw_data[ 1 ] )
	prime_bus_ids = find_prime_bus_numbers( busses )

	# t = 100000000000000
	t = reduce( ( lambda x, y: x * y ), prime_bus_ids )
	# t = 3417
	...
	while found == 0:
		...
		# for bus in busses:
			# output += '\t'
			# if t % bus.id + bus.offset == 0:
				# output += '[{0}] : {1}'.format( bus.id, bus.id * t % 60 )
