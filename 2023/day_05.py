...
class MappingTable():
    ...
    def get_target_value_range(self, seed):
        overlap_range = 0
        results = []
...
def find_location_by_seed_range(seed, data):
    result = []

    current_location_map = data.popleft()
    locations = current_location_map.get_target_value_range(seed)
    if DEBUG: print(f'{current_location_map.id}:\n\t{seed} -> {locations}')

    if data:
        for location in locations:
            x = find_location_by_seed_range(location, copy.deepcopy(data))
            result.append(x)

    else:
        for location in locations:
            x = current_location_map.get_target_value(location[0])
            result.append((x, 0))
    ...
    # for key in RESOURCE_MAP[1:]:
        # # If the key is in the mapping table, look up the new value.
        # if key != RESOURCE_MAP[-1]:
            # result = data[key].get_target_value_range(result)
        # else:
            # for i in range(0,seed[1]):
                # result = seed[0] + i
                # # If the key is in the mapping table, look up the new value.
                # result = data[key].get_target_value(result)
                # results.append(result)

    # if result: results.append(result)

    # # for i in range(0,seed_range[1]):
        # # result = seed_range[0] + i
        # # for key in RESOURCE_MAP[1:]:
            # # # If the key is in the mapping table, look up the new value.
            # # result = data[key].get_target_value(result)
        # # results.append(result)
