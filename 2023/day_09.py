...
def derive_sequence(data):
    result = deque()
    sub_result = deque()
    length = len(data)

    for i in range(0, length - 1):
        sub_result.append(data[i + 1] - data[i])

    result.append(sub_result)
    if sum(sub_result) != 0:
        result = result + derive_sequence(sub_result)
...
def parse_data(raw_data):
    ...
    for datum in raw_data:
        result = derive_sequence(datum)
        result.insert(0, datum)
        data.append(result)

    return data


def calculate_next_number(data):
    # Add up the last numbers for every sequence
    result = sum([x[-1] for x in data])

    return result


def calculate_prev_number(data):
    result = 0
    results = []
    length = len(data)

    if len(data) > 2:
        remainder = data.popleft()
        results = calculate_prev_number(data)
        result = remainder[0] - results[0]
    else:
...
def main(raw_data, part_two = False):
    ...
    for datum in data:
        if not part_two:
            result = calculate_next_number(datum)
            results.append(result)

        else:
            base_num = datum.popleft()
            result = calculate_prev_number(datum)
            results.append(base_num[0] - result[0])

    print(f'\nThe sum of the extrapolated values is {sum(results)}')



if __name__ == "__main__":
    input = r"D:\Projects\Advent_of_Code\2023\day_09_input.txt"
    raw_data = []

    with open(input, "r") as input_file:
        raw_data = deque([deque(map(int,line.split())) for line in input_file.readlines( )])

    # main(TEST_DATA)
    # main(raw_data)
    # main(deque(TEST_DATA), part_two = True)
